...
class AdversarialLayer(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, high_value=1.0, iter_num=0, alpha=10, low=0.0, max_iter=10000.0):
        # 保存需要的信息到 ctx 供 backward 使用
        ...
        # 直接返回输入（identity mapping）
        return input.clone()  # 使用 clone() 避免原地修改

    @staticmethod
    def backward(ctx, gradOutput):
        coeff = np.float32(
            2.0 * (ctx.high - ctx.low) / 
            (1.0 + np.exp(-ctx.alpha * ctx.iter_num / ctx.max_iter)) - 
            (ctx.high - ctx.low) + ctx.low
        )  
        # 更新迭代次数（注意：需要返回与forward参数数量相同的梯度）
        ctx.iter_num += 1
        
        # 返回梯度（对input的梯度，其他参数返回None）
...
class GradientReverseLayer(nn.Module):
    ...
    def forward(self, x):
        # 通过 .apply 调用
        return AdversarialLayer.apply(x, self.high_value)
'''
